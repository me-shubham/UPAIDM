//
//  SellCurrencyViewController.swift
//  UPAIDM
//
//  Created by shubham singh on 27/11/21.
//
...
class SellCurrencyViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBAction func buyRequestTapped(_ sender: Any) {
        ...
        if !checkAccountBalance(selectedCurrency: self.targetAmountLabel.text ?? "") {
            ...
            ApiManager.sharedInstance.sessionByUserId(userId: userId){ [self] (response) in
                ...
                if let response = response {
                    ...
                    if response.status ?? false {
                        ...
                        request.baseAmount = Double(self.amountTextField.text ?? "0.0") ?? 0.0
                        request.baseCurrency = targetCurrency
                        request.password = self.passwordTextField.text ?? ""
                        request.targetAmount = Double(self.baseAmountTextField.text ?? "0.0") ?? 0.0
                        request.targetCurrency = self.baseCurrencyLabel.text ?? ""
    ...
    private func checkAccountBalance(selectedCurrency: String) -> Bool {
        ...
        if selectedCurrency == "MSC1" {
            
            let userBalance =  self.wallet?.MSC1 ?? 0.0
            let baseAmount = Double(self.amountTextField.text ?? "0.0") ?? 0.0
        ...
        if selectedCurrency == "XLM" {
            ...
            let userBalance =  self.wallet?.XLM ?? 0.0
            let baseAmount = Double(self.amountTextField.text ?? "0.0") ?? 0.0
